...
function bindScreenshotButtons(){document.querySelectorAll('[data-shot]').forEach(b=>b.onclick=e=>{e.stopPropagation();let u=b.dataset.shot;if(u)window.open(u,'_blank','noopener')})}
function tradeRow(t){
 ...
 shots=(t.beforeImageUrl||v.afterImageUrl)?`<div class="screenshot-actions">${t.beforeImageUrl?`<button type="button" data-shot="${esc(t.beforeImageUrl)}">View setup screenshot</button>`:''}${v.afterImageUrl?`<button type="button" data-shot="${esc(v.afterImageUrl)}">View after screenshot</button>`:''}</div>`:'',
